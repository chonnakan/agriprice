...
def nearby_search(lat, lng, name):
    ...
    url = url + lat + ',' + lng + '&keyword=' + urllib.quote(name.encode('utf8')) + '&rankby=distance&key=' + master_key
    ...
    if res['status'] == 'OK':
        ...
        while 'next_page_token' in res:
            ...
            url = 'https://maps.googleapis.com/maps/api/place/nearbysearch/json?location='
            url = url + lat + ',' + lng + '&keyword=' + urllib.quote(name.encode('utf8')) + '&rankby=distance&key=' + master_key + '&pagetoken=' + page_token
            #print url
            req = requests.get(url)
            res = json.loads(req.content)
            if res['status'] == 'OK':
